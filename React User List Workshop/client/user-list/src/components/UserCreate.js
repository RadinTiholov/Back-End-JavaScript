...
export const UserCreate = (props) => {
    ...
    const onSubmitHandler = (e) => {
        ...
        //TODO There is a bug(the data above isn't corect in some sense)

        props.onCreateSave(userData);
    }

    const minLength = (e, max) => {
        setErrors(state => ({...state, [e.target.name]: values[e.target.name].length < max}))
    }
    const isPositive = (e) => {
        setErrors(state => ({...state, [e.target.name]: values[e.target.name] < 0}))
    }
    const isValidForm = !Object.values(errors).some(x => x);
    return (
        <div className="overlay">
            <div className="backdrop" onClick={props.closeTab} />
            <div className="modal">
                    ...
                    <form onSubmit={onSubmitHandler}>
                        ...
                        <div className="form-group long-line">
                            ...
                            <div className="input-wrapper">
                                ...
                                <input id="imageUrl" name="imageUrl" type="text" value={values.imageUrl} onChange={changeHandler} onBlur={(e) => minLength(e, 10)}/>
                            </div>
                        ...
                        <div id="form-actions">
                            <button id="action-save" className="btn" type="submit" disabled={!isValidForm}>Save</button>
